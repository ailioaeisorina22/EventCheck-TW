...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    e.preventDefault();
    try {
      await createUserWithEmailAndPassword(auth, email, password); //creem contul in firebase
      const response = await axios.post("http://localhost:9000/api/user", { //trimitem datele la serverul local
    ...
    <div className="d-flex justify-content-center mt-5">
      ...
      <h1 className="text-center text-primary custom-font">Create an account</h1>
        <form onSubmit={handleSignUp}>
          ...
          <button type="submit" className="btn btn-primary btn-lg rounded-pill col-11 mt-2"style={{ backgroundColor: '#a91f1f' }}>
            Sign Up
          </button>
        </div>
        </form>
