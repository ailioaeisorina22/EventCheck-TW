...
import UserEventChoose from "../UserEventChoose/UserEventChoose"; 
...
const HomeUser = () => {
 ...
 const checkEventStatus = (event) => {
  ...
  return "CLOSED"; // e implicit, până a început
};
    ...
    if(response.data.events.length!=0)
    {
      ...
      const eventResponse = response.data.events.map((event) => {
        ...
        return {
          ...
          meetingOption: checkEventStatus({
            eventDateStart: new Date(event.EventStartDate),
            eventDateEnd: new Date(event.EventEndDate),
          }),
          repeatOption: "Never",
    ...
    onAuthStateChanged(auth, (user) => {
      ...
      const fetchData = async () => {
        try {
          ...
          const updatedEvents = eventResponse.data.events.map((event) => {
            let startHours = new Date(event.EventStartDate).getHours();
            let startMinutes = new Date(event.EventStartDate).getMinutes();
            let endHours = new Date(event.EventEndDate).getHours();
            let endMinutes = new Date(event.EventEndDate).getMinutes();

            const updatedStatus = checkEventStatus({
  ...
  return (
    <div className="container">
      ...
      <div className="container-evenimente">
        <button className="red-button" onClick={openUserEventChooseModal}>
          Join Event
        </button>
        <UserEventChoose
