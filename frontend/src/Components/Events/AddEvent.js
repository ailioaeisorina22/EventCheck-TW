...
function AddEvent(props) {
  ...
  const calculateDuration = () => {
    if (startTime && endTime) {
      const start = new Date(`January 01, 2025 ${startTime}`);
      const end = new Date(`January 01, 2025 ${endTime}`);
  ...
  return (
    <div className="add-event-container">
      ...
      <div className="add-event-header">Add an event:</div>
      <form onSubmit={handleSave}>
        <div className="add-event-body">
          ...
          <div className="add-event-notice">
            The event will take place for{" "}
            {repeatOption === "Never" ? 1 : repeatDays} days, from:{" "}
